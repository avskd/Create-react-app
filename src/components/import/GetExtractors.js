...
class GetExtractors extends React.Component {
    ...
    componentDidMount() {
        ...
        fetch(`https://api.import.io/maestro/extractor?_apikey=${food_api}`)
            .then(res => res.json())
            .then((json) => {
                console.log(json)
                let jsonData = json.map(extractor => {
                    return {
                        name: extractor.name,
                        guid: extractor.guid,
                        duplicateOfExtractorId: extractor.duplicateOfExtractorId,
                        isScheduled: extractor.isScheduled
                    }
                });
                this.setState({
                    users: jsonData
            ...
            rows: [...this.state.users.map((data, i) => (
                {
                    ...
                    isScheduled: data.isScheduled === "false" ? "FALSE" : "TRUE"
                }
            ))]
        };
            ...
            <div className="flex-1 flex flex-col bg-white overflow-hidden">
                ...
                <div className="border-b flex px-6 py-2 items-center flex-none">
                    <div className="flex flex-col">
                        <h3 className="text-grey-darkest mb-1 font-extrabold">Get Extractor Details</h3>
                    </div>
